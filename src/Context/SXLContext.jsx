...
export const SXLContext = createContext();

export const SXLContextProvider = ({children}) => {

    const [Sxloffduty, setSxlOffduty] = useState([])
    const [SxlMaintenance, setSxlMaintenance] = useState([])
    const [SxlIntransit, setSxlIntransit] = useState([])
    const [SxlLoading, setSxlLoading] = useState([])
    const [Sxlcount, setSxlCount] = useState(0);
    ...
    const PorocessData = async(val) => {
        switch(val.status) {
            case "OffDuty":
                setSxlOffduty((pre) => {
                    pre.push(SxlDataSelection(val));
                    return pre;
                }
                );
                break;
            case "Maintenance":
                setSxlMaintenance((pre)=> [...pre, SxlDataSelection(val)]);
                break;
            case "Enroute for delivery":
                setSxlIntransit((pre)=> [...pre, SxlDataSelection(val)]);
                break;
            case "At pickup":
                setSxlLoading((pre)=> [...pre, SxlDataSelection(val)]);
                break;
            default:
                break;
    ...
    async function fetchStatusDataFromSxlApmlApi() {
        const headers = {
        ...
        setSxlCount(response.data.data.count);
        DATA.map((val, index) => {
            if(JSON.stringify(val).toLocaleLowerCase().includes("sxl")) PorocessData(val);
        })  
    }
    useEffect(()=> {
